...
外部依存はなく、ボタン、札（カード）、欄（入力欄）、織り（パネル）の4つの基本コンポーネントで構成される。
...
ボタン、札、欄、織りは、すべて同一の描画関数 `paint()` によって描かれる。
...
| レイヤー | 役割 | 適用コンポーネント |
|---|---|---|
| `tz-shade` | 接地影。ぼかしを加えた同一輪郭 | 札 |
| `tz-fill` | 面の塗り（素地） | ボタン、枡、織り |
| `tz-press` | 指で押した位置から広がるへこみの陰影 | ボタン、札 |
| `tz-grain` | 地肌の繊維テクスチャ | ボタン、札、織り |
| `tz-edge2` | 筆圧の表現。太い破線を輪郭線に重ねる | ボタン、札 |
| `tz-soak` | 記入された文字の墨の染み込み | 欄 |
| `tz-edge` | 輪郭線そのもの | 全コンポーネント |
| `tz-correct` | 朱色の訂正波線 | 欄 |
| `tz-hook` | かぎ（書ける範囲の両端を示す跳ね） | 欄 |
| `tz-fringe` | 紙の耳（毛羽立ち） | 札 |
| `tz-ring` | フォーカスリング | ボタン、札 |
...
織りコンポーネントの `--tz-grain-freq` のみ、横方向と縦方向の2つの値を指定する。
...
`--tz-sigma` の値が、ボタンと札の挙動の違いを決定付ける。
ボタンは短辺の 0.55 倍に設定されているため、押下された局所的な一点のみがへこむ。
一方の札は 1.7 倍と広く取られており、一点がへこむのではなく紙一枚全体がしなるように変形する。
実測値として、幅 668px の札の左端を押したとき、手前の縁が 2.29px、対向する縁が 1.72px 変位する。
...
枠で囲む必要があるのは短い記号や数値を扱う場合のみであり、その際は枡型（`--boxed`）を用いる。
...
持ち上げ可能な札（`--pick`）では `arrive` を 0 としている。
...
ボタンは1本の線が引かれるように現れ、札は机の上に紙が置かれるように着地し、欄は罫線が引かれるように出現する。
所要時間は個体ごとに 540ms〜960ms の間で分散させ、46ms ずつ開始タイミングをずらしている。

札のテキスト内容は、紙が着地した後に初めて読み取れるようになる。
...
ボタンと札は、輪郭の外側に手描き風の環（フォーカスリング）を描く。
...
| 枡および原稿用紙 | 線幅 1.15px / 不透明度 0.38 | 線幅 1.9px / 不透明度 1.0 |
